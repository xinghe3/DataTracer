...
namespace DC.ETL.Infrastructure.Cache.Redis
{
    ...
    /// 功能调用全部通过<see cref="RedisCacheProvider"/>进行
    ...
    public class RedisUtils : IDisposable
    {
        ...
        {
            if (!IsConnMultiplexerConnected() || !IsDBConnected())
            {
                lock (Locker)
                {
                    if (!IsConnMultiplexerConnected() || !IsDBConnected())
                    {
                        //if (_connMultiplexer != null) _connMultiplexer.Dispose();
                        _connMultiplexer = Connect();
                    }
                }
            }
        ...
        {
            return (_db != null) && (_db.Multiplexer != null) && _db.Multiplexer.IsConnected;
        }

        private bool IsConnMultiplexerConnected()
        {
            return (_connMultiplexer != null) && _connMultiplexer.IsConnected;
        }

        #region 其它
        private ConfigurationOptions BuildConfigurationOptions()
        {
            ConfigurationOptions conf = ConfigurationOptions.Parse(cacheSetting.Connection);
        ...
        private IConnectionMultiplexer Connect()
        {
            ...
            try
            {
                IConnectionMultiplexer _connMultiplexer = ConnectionMultiplexer.Connect(cacheSetting.Connection);
                AddRegisterEvent(_connMultiplexer);
                return _connMultiplexer;
            }
            catch (Exception ex)
            {
                Logs.WriteExLog( ex, "RedisConnectionString:[", cacheSetting.Connection, "] Redis.DefaultKey: ", DefaultKey);
            }
            return null;
        }

        public RedisUtils(CacheSetting cacheSetting)
        {
        ...
        public bool StringSet(string redisKey, string redisValue, TimeSpan? expiry = null)
        {
            ...
            return GetDB().StringSet(redisKey, redisValue, expiry);
        }
        ...
        public bool StringSet(IEnumerable<KeyValuePair<RedisKey, RedisValue>> keyValuePairs)
        {
            ...
            return GetDB().StringSet(keyValuePairs.ToArray());
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return GetDB().StringGet(redisKey);
        }
        ...
        public bool StringSet<T>(string redisKey, T redisValue, TimeSpan? expiry = null)
        {
            ...
            return GetDB().StringSet(redisKey, json, expiry);
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return Deserialize<T>(GetDB().StringGet(redisKey));
        }
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            return await GetDB().StringSetAsync(redisKey, redisValue, expiry);
        }
        ...
        public bool HashSet<T>(string redisKey, string hashField, T value)
        {
            ...
            try
            {
                ...
                return GetDB().HashSet(redisKey, hashField, json);
            }
            catch (Exception ex)
            {
                Logs.WriteExLog( ex, redisKey, hashField);
        ...
        public T HashGet<T>(string redisKey, string hashField)
        {
            ...
            try
            {
                var result = GetDB().HashGet(redisKey, hashField);
                return Deserialize<T>(result);
                //var result = GetDB().HashGet(redisKey, hashField);
                //return (T)CombineData<T>(redisKey, hashField);
            }
            catch (Exception ex)
            {
                Logs.WriteExLog( ex, redisKey, hashField);
            ...
            if (!IsConnMultiplexerConnected() || !IsDBConnected())
            {
                ...
                do
                {
                    ...
                    if (_connMultiplexer != null)
                    {
                        try
                        {
                            _connMultiplexer.GetDatabase(-1).KeyExists("1");
                        }
                        catch
                        {
                            _connMultiplexer.Dispose();
                            _connMultiplexer = null;
                        }
                    }

                    n++;
                } while (_connMultiplexer == null && n < 10);
        ...
        /// 添加注册事件
        /// </summary>
        private static void AddRegisterEvent(IConnectionMultiplexer _connMultiplexer)
        {
